...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
    ...
    private void initialize(GL10 gl) {
        ...
        GLES32.glBindVertexArray(vao_pyramid[0]);
            ...
            ByteBuffer byteBuffer_pyramid = ByteBuffer.allocateDirect(pyramidVertices.length*4);
            byteBuffer_pyramid.order(ByteOrder.nativeOrder());
            FloatBuffer verticesBuffer_pyramid = byteBuffer_pyramid.asFloatBuffer();
            verticesBuffer_pyramid.put(pyramidVertices);
            verticesBuffer_pyramid.position(0);
            //_______________________________________________________________________

            GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, pyramidVertices.length * 4, verticesBuffer_pyramid, GLES32.GL_STATIC_DRAW);
        ...
        GLES32.glBindVertexArray(vao_cube[0]);
            ...
            //_______________________________________________________________________
            ByteBuffer byteBuffer_cube = ByteBuffer.allocateDirect(cubeVertices.length*4);
            byteBuffer_cube.order(ByteOrder.nativeOrder());
            FloatBuffer verticesBuffer_cube = byteBuffer_cube.asFloatBuffer();
            verticesBuffer_cube.put(cubeVertices);
            verticesBuffer_cube.position(0);
            //_______________________________________________________________________

            GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, cubeVertices.length * 4, verticesBuffer_cube, GLES32.GL_STATIC_DRAW);
            GLES32.glVertexAttribPointer(GLESMacros.RSK_ATTRIBUTE_VERTEX, 3, GLES32.GL_FLOAT, false,0,0);
            GLES32.glEnableVertexAttribArray(GLESMacros.RSK_ATTRIBUTE_VERTEX);
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER,0);
    ...
    private void display() {
        ...
        Matrix.translateM(modelViewMatrix, 0, modelViewMatrix, 0, -1.75f, 0.0f, -5.0f);
