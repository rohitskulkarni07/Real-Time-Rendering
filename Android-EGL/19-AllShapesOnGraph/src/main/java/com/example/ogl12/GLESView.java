...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
    ...
    float gfRadOuter = (float) (Math.sqrt(Math.pow(0.5f - 0.0f, 2) + Math.pow(0.5f - 0.0f, 2)));
    ...
    private void initialize(GL10 gl) {
        ...
        float[] outerCircleVertices = new float[1080];
        angleOfCircle = 0.0f;

        for(int i = 0; i < 360; i++) {
            outerCircleVertices[i*3]     = gfRadOuter * (float) Math.cos(angleOfCircle);
            outerCircleVertices[i*3 + 1] = gfRadOuter * (float) Math.sin(angleOfCircle);
            outerCircleVertices[i*3 + 2] = 0.0f;
            angleOfCircle += 2 * Math.PI/360;
        }
        ...
        //                          VAO_OUTER_CIRCLE
        //_______________________________________________________________________
        GLES32.glGenVertexArrays(1, vao_outer_circle, 0);
        GLES32.glBindVertexArray(vao_outer_circle[0]);

            //_______________________________________________________________________
            //                          POSITION
            //_______________________________________________________________________
            GLES32.glGenBuffers(1, vbo_position_outer_circle, 0);
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_position_outer_circle[0]);

            //_______________________________________________________________________
            byteBuffer = ByteBuffer.allocateDirect(outerCircleVertices.length*4);
            byteBuffer.order(ByteOrder.nativeOrder());
            verticesBuffer = byteBuffer.asFloatBuffer();
            verticesBuffer.put(outerCircleVertices);
            verticesBuffer.position(0);
            //_______________________________________________________________________

            GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, outerCircleVertices.length * 4, verticesBuffer, GLES32.GL_STATIC_DRAW);
            GLES32.glVertexAttribPointer(GLESMacros.RSK_ATTRIBUTE_VERTEX, 3, GLES32.GL_FLOAT, false, 0, 0);
            GLES32.glEnableVertexAttribArray(GLESMacros.RSK_ATTRIBUTE_VERTEX);
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER,0);
    ...
    private void display() {
        ...
        Matrix.scaleM(modelViewMatrix,0,modelViewMatrix,0, 1.0f, 0.5f, 0.0f);
        Matrix.multiplyMM(modelViewProjectionMatrix,0,perspectiveProjectionMatrix,0,modelViewMatrix,0);

        drawLines(modelViewProjectionMatrix, color_yellow);
    ...
    private void drawTriangle() {
        ...
        //scale
        Matrix.scaleM(modelViewMatrix,0,modelViewMatrix,0, 0.5f, 0.5f, 0.0f);
    ...
    private void drawCircle() {
        ...
        // translate
        Matrix.translateM(modelViewMatrix,0,modelViewMatrix,0, ix, iy, -2.5f);
