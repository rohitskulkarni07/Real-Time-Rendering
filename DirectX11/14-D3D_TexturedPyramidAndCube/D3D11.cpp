...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_CubePosition = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_CubeTexCoords = NULL;

ID3D11Buffer *gpID3D11Buffer_VertexBuffer_PyramidPosition = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_PyramidTexCoords = NULL;
...
// *Kundali Texture
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_KundaliTexture = NULL; //* resource being use for kundali texture
ID3D11SamplerState *gpID3D11SamplerState_KundaliTexture = NULL;             //* create a sampler-state object that encapsulates sampling information for a kundali texture
// *Pyramid Texture
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_PyramidTexture = NULL; //* resource being use for pyramid texture
ID3D11SamplerState *gpID3D11SamplerState_PyramidTexture = NULL;             //* create a sampler-state object that encapsulates sampling information for a pyramid texture
...
HRESULT initialize(void)
{
    ...
    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
                                      NULL,
                                      &gpID3D11Buffer_VertexBuffer_CubePosition);
    ...
    // *cpu to gpu mapping of data
    D3D11_MAPPED_SUBRESOURCE mappedSubResource;
    ZeroMemory((void *)&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_CubePosition,
                               0,
                               D3D11_MAP_WRITE_DISCARD,
                               0,
                               &mappedSubResource);

    // *actual copy of done here from cpu to gpu
    memcpy(mappedSubResource.pData, cubeVertices, sizeof(cubeVertices));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_CubePosition, 0);
    ...
    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
                                      NULL,
                                      &gpID3D11Buffer_VertexBuffer_PyramidPosition);
    ...
    // *cpu to gpu mapping of data
    ZeroMemory((void *)&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_PyramidPosition,
                               0,
                               D3D11_MAP_WRITE_DISCARD,
                               0,
                               &mappedSubResource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_PyramidPosition, 0);
    ...
    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
                                      NULL,
                                      &gpID3D11Buffer_VertexBuffer_CubeTexCoords);
    ...
    ZeroMemory((void *)&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_CubeTexCoords,
                               0,
                               D3D11_MAP_WRITE_DISCARD,
                               0,
                               &mappedSubResource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_CubeTexCoords, 0);
    //__________________________________________________________________________
    // TexCoords for pyramid
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    ...
    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
                                      NULL,
                                      &gpID3D11Buffer_VertexBuffer_PyramidTexCoords);
    ...
    ZeroMemory((void *)&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_PyramidTexCoords,
                               0,
                               D3D11_MAP_WRITE_DISCARD,
                               0,
                               &mappedSubResource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_PyramidTexCoords, 0);
    ...
    hr = LoadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_KundaliTexture);
    if (FAILED(hr))
    {
        fopen_s(&gpFile, gszFileName, "a+");
    ...
    hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDesc, &gpID3D11SamplerState_KundaliTexture);
    if (FAILED(hr))
    {
        fopen_s(&gpFile, gszFileName, "a+");
    ...
    // Load Texture Pyramid
    hr = LoadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_PyramidTexture);
    if (FAILED(hr))
    {
        fopen_s(&gpFile, gszFileName, "a+");
        fprintf_s(gpFile, "LoadD3DTexture::LoadD3DTexture() Failed!\n");
    ...
    hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDesc, &gpID3D11SamplerState_PyramidTexture);
    if (FAILED(hr))
    {
        fopen_s(&gpFile, gszFileName, "a+");
        fprintf_s(gpFile, "gpID3D11Device::CreateSamplerState() Failed!\n");
...
{
    HRESULT hr = DirectX::CreateWICTextureFromFile(gpID3D11Device, gpID3D11DeviceContext, textureFileName, NULL, ppID3D11ShaderResourceView); /* For resource loading instead of file we can use CreateWICTextureFromMemory */
    if (FAILED(hr))
    {
        fopen_s(&gpFile, gszFileName, "a+");
...
void display(void)
{
    ...
    // *Transformations
    XMMATRIX translationMatrix = XMMatrixTranslation(2.0f, 0.0f, 6.0f);

    XMMATRIX scaleMatrix = XMMatrixScaling(0.78f, 0.78f, 0.78f);

    XMMATRIX rotationMatrix_X = XMMatrixRotationX(gAngle);
    XMMATRIX rotationMatrix_Y = XMMatrixRotationY(gAngle);
    XMMATRIX rotationMatrix_Z = XMMatrixRotationZ(gAngle);

    XMMATRIX worldMatrix = rotationMatrix_X * rotationMatrix_Y * rotationMatrix_Z * scaleMatrix * translationMatrix;
    ...
    gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_KundaliTexture);

    // *Pass the sampler to pixel shader
    gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_KundaliTexture);
    ...
    rotationMatrix_Y = XMMatrixRotationY(gAngle);
    worldMatrix = rotationMatrix_Y * translationMatrix;
    ...
    // *Pass the sampler to pixel shader
    gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_PyramidTexture);
...
void uninitialize(void)
{
    ...
    if (gpID3D11SamplerState_KundaliTexture)
    {
        gpID3D11SamplerState_KundaliTexture->Release();
        gpID3D11SamplerState_KundaliTexture = NULL;
    }
    if (gpID3D11ShaderResourceView_KundaliTexture)
    {
        gpID3D11ShaderResourceView_KundaliTexture->Release();
        gpID3D11ShaderResourceView_KundaliTexture = NULL;
    }
    ...
    if (gpID3D11Buffer_VertexBuffer_CubePosition)
    {
        gpID3D11Buffer_VertexBuffer_CubePosition->Release();
        gpID3D11Buffer_VertexBuffer_CubePosition = NULL;
    }

    if (gpID3D11Buffer_VertexBuffer_CubeTexCoords)
    {
        gpID3D11Buffer_VertexBuffer_CubeTexCoords->Release();
        gpID3D11Buffer_VertexBuffer_CubeTexCoords = NULL;
    }
