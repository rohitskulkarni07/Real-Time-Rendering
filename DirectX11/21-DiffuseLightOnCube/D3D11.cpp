...
HRESULT initialize(void)
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "       float3 transformedNormal = normalize(mul((float3x3)worldViewMatrix, (float4)normal));       \n"
        "       float3 lightSourceVector = (float3) normalize((lightPosition - eyePosition));               \n"
        "       output.diffuse_light = ld * kd * max(dot(lightSourceVector, transformedNormal), 0.0);       \n"
        "   } else {                                                                                        \n"
        "       output.diffuse_light = float4(1,1,1,1);                                                     \n"
        "   }                                                                                               \n"
