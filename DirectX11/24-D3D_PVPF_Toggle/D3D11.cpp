...
HRESULT initialize(void)
{
    ...
    const char *vertexShaderSourceCode_PP =
        ...
        "       output.lightDirection = (float3) (lightPosition) - eyePosition.xyz;                                     \n"
        "       output.viewerVector = -eyePosition.xyz;                                                                 \n"
        "                                                                                                               \n"
        "   }                                                                                                           \n"
        "   float4 position = mul(worldMatrix, pos);                                                                    \n"
        "   position = mul(viewMatrix, position);                                                                       \n"
    ...
    const char *pixelShaderSourceCode_PP =
        ...
        "   if(key_pressed == 1) {                                                                                              \n"
        "                                                                                                                       \n"
        "       float3 normalized_transformedNormal = normalize(input.transformedNormal);                                       \n"
        ...
        "                                                                                                                       \n"
        "       float3 ambient = la * ka;                                                                                       \n"
        "       float3 diffuse = ld * kd * max(dot(normalized_lightDirection, normalized_transformedNormal), 0.0);              \n"
        "       float3 reflectionVector = reflect(-normalized_lightDirection, normalized_transformedNormal);                    \n"
        "       float3 specular = ls * ks * pow(max(dot(reflectionVector, normalized_viewerVector), 0.0), materialShinnines);   \n"
    ...
    ID3D10Blob *pID3DBlob_PixelShaderCode_PP = NULL;
    pID3DBlob_Error_PP = NULL;
    ...
    // *Sequence Changes As Per Sirs Suggestion
    D3D11_INPUT_ELEMENT_DESC inputElementDesc_PP[2];

    // Position
    inputElementDesc_PP[0].SemanticName = "POSITION";
    inputElementDesc_PP[0].SemanticIndex = 0;
    inputElementDesc_PP[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;
    inputElementDesc_PP[0].AlignedByteOffset = 0;
    inputElementDesc_PP[0].InputSlot = 0;
    inputElementDesc_PP[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
    inputElementDesc_PP[0].InstanceDataStepRate = 0;

    inputElementDesc_PP[1].SemanticName = "NORMAL";
    inputElementDesc_PP[1].SemanticIndex = 0;
    inputElementDesc_PP[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;
    inputElementDesc_PP[1].AlignedByteOffset = 0;
    inputElementDesc_PP[1].InputSlot = 1;
    inputElementDesc_PP[1].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
    inputElementDesc_PP[1].InstanceDataStepRate = 0;

    hr = gpID3D11Device->CreateInputLayout(inputElementDesc_PP,
                                           _ARRAYSIZE(inputElementDesc_PP),
    ...
    //_______________________________________________________________________
    // Initialise and create constant buffer
    D3D11_BUFFER_DESC bufferDesc_ConstantBuffer_PP;
    ZeroMemory(&bufferDesc_ConstantBuffer_PP, sizeof(D3D11_BUFFER_DESC));
    bufferDesc_ConstantBuffer_PP.Usage = D3D11_USAGE_DEFAULT;
    bufferDesc_ConstantBuffer_PP.ByteWidth = sizeof(CBUFFER);
    bufferDesc_ConstantBuffer_PP.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    bufferDesc_ConstantBuffer_PP.CPUAccessFlags = 0;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_PP,
                                      nullptr,
                                      &gpID3D11Buffer_ConstantBuffer_PP);
    if (FAILED(hr))
    {
        fopen_s(&gpFile, gszFileName, "a+");
        fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed!\n");
...
void display(void)
{
    ...
    if (gbLights)
    {
        ...
        else
        {
            ...
            gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_PP,
                                                     0,
                                                     NULL,
                                                     &constantBuffer,
                                                     0,
