...
struct CBUFFER
{
    ...
    XMVECTOR LA[2]; // ambient  component of light
    XMVECTOR LD[2]; // diffuse  component of light
    XMVECTOR LS[2]; // specular component of light
    ...
    XMVECTOR LightPosition[2]; // lights position
...
HRESULT initialize(void)
{
    ...
    const char *vertexShaderSourceCode_PV =
        ...
        "       for(i = 0; i < 2; i++) {                                                                                                       \n"
        "                                                                                                              \n"
        "           float3 lightDirection = (float3) normalize((lightPosition[i] - eyePosition));                              \n"
        "           float3 reflectionVector = reflect(-lightDirection, transformedNormal);                                  \n"
        "                                                                                                              \n"
        "           float3 ambient = la[i] * ka;                                                                               \n"
        "           float3 diffuse = ld[i] * kd * max(dot(lightDirection, transformedNormal), 0.0);                            \n"
        "           float3 specular = ls[i] * ks * pow(max(dot(reflectionVector, viewerVector), 0.0), materialShinnines);      \n"
   ...
   gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayout_PV);
    ...
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidNormals);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
...
void display(void)
{
    ...
    // *Transformations
    XMMATRIX translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 4.5f);

    // *Load the data into constant buffer
    CBUFFER constantBuffer;
    constantBuffer.WorldMatrix = XMMatrixRotationY(gAngle) * translationMatrix;;
