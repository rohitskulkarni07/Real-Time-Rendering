...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_PCNT = NULL;
...
HRESULT initialize(void)
{
    ...
    const char *pixelShaderSourceCode =
        ...
        "                                                                                                                       \n"
        "       float3 ambient = la.xyz * ka.xyz;                                                                                       \n"
        "       float3 diffuse = ld.xyz * kd.xyz * max(dot(normalized_lightDirection, normalized_transformedNormal), 0.0);              \n"
        "       float3 reflectionVector = reflect(-normalized_lightDirection, normalized_transformedNormal);                    \n"
        "       float3 specular = ls.xyz * ks.xyz * pow(max(dot(reflectionVector, normalized_viewerVector), 0.0), materialShinnines);   \n"
        "       phong_ads_light = float4(ambient + diffuse + specular, 1.0);                                                                 \n"
        "                                                                                                                       \n"
        "   } else {                                                                                                            \n"
        ...
        "                                                                                                                         \n"
        "   if(key_pressed & 2) {                                                                                                                  \n"
        "       phong_ads_light = phong_ads_light * input.color;                                                                          \n"
        "   }                                                                                                                  \n"
        "   if(key_pressed & 4) {                                                                                                                  \n"
        "       float4 texColor = myTexture2D.Sample(mySamplerState, input.uv);                                                                                                                    \n"
        "       phong_ads_light = phong_ads_light * texColor;                                                                          \n"
        "   }                                                                                                                  \n"
    ...
    bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_pcnt);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
                                      NULL,
                                      &gpID3D11Buffer_VertexBuffer_PCNT);
    ...
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_PCNT,
                               0,
                               D3D11_MAP_WRITE_DISCARD,
                               0,
                               &mappedSubResource);

    memcpy(mappedSubResource.pData, cube_pcnt, sizeof(cube_pcnt));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_PCNT, 0);
...
void display(void)
{
    ...
    XMMATRIX worldMatrix = rotationMatrix1 * rotationMatrix1 * rotationMatrix3 * translationMatrix;
    ...
    constantBuffer.KeyPressed = keyPress;
...
void uninitialize(void)
{
    ...
    if (gpID3D11Buffer_VertexBuffer_PCNT)
    {
        gpID3D11Buffer_VertexBuffer_PCNT->Release();
        gpID3D11Buffer_VertexBuffer_PCNT = NULL;
    }
