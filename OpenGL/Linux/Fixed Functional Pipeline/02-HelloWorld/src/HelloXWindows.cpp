...
#include <X11/keysym.h> // KeySym Releted API (Keyboard Event)
...
int main(void) {
	...
	static XFontStruct *pXFontStruct; 	// For Font
	static GC gc;				//Softwere Graphics Contex Display Structure
	
	XGCValues gcValues;			// Similar to Paint Struct
	XColor greenColor;
	...
	// Message Loop
	XEvent event;		// Event To Be Capture -> similar to MSG msg In Win32
	KeySym keysym;		// KeySym Key Symbols Keyboard Keys has Give Symbols By System
	...
	while(1) {
		...
		switch(event.type) {
			...
			case KeyPress:	//key-pressed event
...
void CreateWindow(void) {
	...
	// Variable Declarations
	XSetWindowAttributes winAttribs; //Object To struct which Holds Information about Window Atrributes
	...
	*/	
	...
	// Information Releted to Visual Information screen, depth, color
	...
	*/
	...
	Event Masks:
		...
		KeyPressMask			Keyboard down events wanted (keyboard key press)
		KeyReleaseMask			Keyboard up events wanted
		ButtonPressMask			Pointer button down events wanted(mouse click)
		...
		VisibilityChangeMask		Any change in visibility wanted (max, min , overlapped, etc, hide and close)
		StructureNotifyMask		Any change in window structure wanted(resize)
