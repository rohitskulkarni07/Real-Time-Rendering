...
void initialize(void) {
	...
	gui_stone = LoadBitmapAsTexture("Stone.bmp");
	gui_kundali = LoadBitmapAsTexture("Vijay_Kundali.bmp");
...
void display(void) {
	...
	glBindTexture(GL_TEXTURE_2D,gui_stone);	//change state to texture for next upcoming geometry
	drawPyramid();		
	...
	glBindTexture(GL_TEXTURE_2D,gui_kundali);	//change state to texture for next upcoming geometry
	drawCube();		
...
void uninitialize(void) {
	...
	if(gui_stone) {
		glDeleteTextures(1, &gui_stone);
		gui_stone = 0;
	}

	if(gui_kundali) {
		glDeleteTextures(1, &gui_kundali);
		gui_kundali = 0;
	}
