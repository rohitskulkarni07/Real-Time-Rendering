...
GLfloat material_ambient[] = { 0.8, 0.0, 0.0, 1.0 };
				...
				case KeyPress:
					...
					switch(keysym) {
						...
						case XK_l:
							...
							case 'A':
								gfCutoff_angle -=1;
								printf("cutoff = %f, ecxpo = %f\n",gfCutoff_angle, gfExpo);
								break;
...
// Function Prototype
	...
	glLightf(GL_LIGHT1, GL_SPOT_CUTOFF, gfCutoff_angle);
	glLightf(GL_LIGHT1, GL_SPOT_EXPONENT, gfExpo);
...
glBegin(GL_QUADS);
	...
	*/
	...
	for (float f = -8.0f; f<=8.0f; f+=1.0f){

		glBegin(GL_TRIANGLE_STRIP);
		glNormal3f(0,1,0);

		for (float fz = 8.0f;fz>=-8.0f; fz-=1.0f) {
		glVertex3f(f,0,fz);
		glVertex3f(f+1.0f,0,fz);

		}
		glEnd();
	}
	//Here, For Lights Normal Are Calculated By glu Itself Hence No Manual Calculation Of Normals By Explicitly
