...
void initialize(void) {
	...
	"																				\n" \
	"void main(void) {																\n" \
	"	FragColor = vec4(1.0f,1.0,1.0,1.0);														\n" \
	"}																				\n";
...
void display(void) {
	...
	glUseProgram(shaderProgramObject);
		...
		rotationMatrix 		= rotate(pyramidAngle, 0.0f,1.0f,0.0f);
	...
	scalingMatrix				= mat4::identity();	
		...
		translationMatrix 	= translate(1.5f,0.0f,-6.0f);
		rotationMatrix 		= rotate(cubeAngle, 1.0f,0.0f,0.0f) * rotate(cubeAngle, 0.0f,1.0f,0.0f); * rotate(cubeAngle, 1.0f,0.0f,1.0f);
		scalingMatrix		= scale(0.75f, 0.75f, 0.75f);
		modelViewMatrix 	= translationMatrix * scalingMatrix * rotationMatrix;
