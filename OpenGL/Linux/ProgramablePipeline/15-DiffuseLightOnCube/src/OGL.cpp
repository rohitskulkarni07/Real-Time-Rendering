...
GLuint		uniform_modelViewMatrix;
GLuint		uniform_perspectiveProjectionMatrix;
GLuint		uniform_lkeypressed;

GLuint		uniform_ld;						// light diffuse component
GLuint		uniform_kd;						// material diffuse component
GLuint		uniform_lightPosition;			// position of light
...
void initialize(void) {
	...
	uniform_modelViewMatrix					= glGetUniformLocation(shaderProgramObject, "u_model_view_matrix");
	uniform_perspectiveProjectionMatrix		= glGetUniformLocation(shaderProgramObject, "u_projection_matrix");
	uniform_lkeypressed 					= glGetUniformLocation(shaderProgramObject, "u_lkeypressed");
...
void display(void) {
	...
	glUseProgram(shaderProgramObject);
		...
		//____________________________________________________________________________________________________
		if(gbLights) {
			glUniform1i(uniform_lkeypressed, 1);
			glUniform3f(uniform_ld, 1.0f,1.0f,1.0f);			// light diffuse
			glUniform3f(uniform_kd, 0.5f,0.5f,0.5f);			// material

			GLfloat light_position[] = {0.0f,0.0f,2.0f,1.0f};	// light position
			glUniform3fv(uniform_lightPosition, 1, (GLfloat*)light_position);
		} else {
			glUniform1i(uniform_lkeypressed, 0);
		}
		...
		rotationMatrix 		= rotate(cubeAngle, 1.0f,0.0f,0.0f) * rotate(cubeAngle, 0.0f,1.0f,0.0f); * rotate(cubeAngle, 0.0f,0.0f,1.0f);;
		...
		glUniformMatrix4fv(uniform_perspectiveProjectionMatrix,1, GL_FALSE, projectionMatrix);
