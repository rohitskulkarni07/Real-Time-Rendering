...
void display(void) {
	...
	// function prototype
	void update (void);
 	void draw(mat4 modelViewMatrix, GLfloat[]);
	// code
	...
	pushMatrix(modelViewMatrix);
		draw(modelViewMatrix, yellow);
	popMatrix(&modelViewMatrix);

	//EARTH
	pushMatrix(modelViewMatrix);
		modelViewMatrix = modelViewMatrix * vmath::rotate((float)year, 0.0f,1.0f,0.0f) * vmath::translate(1.0f, 0.0f, 0.0f) * vmath::scale(0.35f,0.35f,0.35f) *vmath::rotate(90.0f, 1.0f, 0.0f, 0.0f) * vmath::rotate((float)day, 0.0f, 0.0f, 1.0f);
		draw(modelViewMatrix, cyan);

		//MOON
		pushMatrix(modelViewMatrix);
			modelViewMatrix = modelViewMatrix * vmath::translate(1.0f, 0.0f, 0.0f) * vmath::scale(0.20f, 0.20f, 0.20f) * vmath::rotate((float)month, 0.0f, 0.0f, 1.0f);
			draw(modelViewMatrix, gray);
		popMatrix(&modelViewMatrix);
...
void draw(mat4 modelViewMatrix,GLfloat color[]) {
...
/*
	COMPILE: 	g++ -o OGL OGL.cpp MyStack.cpp -lX11 -lGL -lGLEW -L"./" -lSphere
