...
//	get uniform locations from shader program
	...
	glBindFramebuffer(GL_FRAMEBUFFER, framebufferObject);
		...
		// Depth buffer
		glGenRenderbuffers(1, &depthTextureAttachment);
		glBindRenderbuffer(GL_RENDERBUFFER, depthTextureAttachment);
		...
		glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthTextureAttachment); 

		if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
			printf("Failed to create framebuffer! exiting now...!");
			uninitialize();
			exit(0);
		}
...
void display(void) {
	...
	translateMatrix = vmath::translate(0.0f, 0.0f, -3.5f);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, colorTextureAttachment);	//change state to texture for next upcoming geometry
	glUniform1i(textureSamplerUniform, 0);
...
/*
	COMPILE: 	g++ -o OGL OGL.cpp -lX11 -lGL -lGLEW -lSOIL
	RUN: 		./OGL
