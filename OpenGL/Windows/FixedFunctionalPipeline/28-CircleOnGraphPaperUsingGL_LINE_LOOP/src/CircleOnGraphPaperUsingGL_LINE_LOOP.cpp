...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,	//newly added parametr Beacuse CreateWindowEx
		szAppName,
		TEXT("Circle On Graph Paper Using GL_LINE_LOOP"),
...
void DrawCircle(void) {
	...
	static int iCirclePoints = 1000;

	glBegin(GL_LINE_LOOP);
	
	// 1000 is points

	for (int angle = 0; angle <= iCirclePoints; angle++) {
	
		glVertex3f(0.3*cos(2*PI*angle/iCirclePoints), 0.3*sin(2 * PI * angle / iCirclePoints), 0.0f);
	}
