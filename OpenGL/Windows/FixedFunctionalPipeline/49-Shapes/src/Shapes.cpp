...
void display(void) {
	...
	void FigureOne(void);
	void FigureTwo(void);
	void FigureThree(void);
	void FigureFour(void);
	void FigureFive(void);
	void FigureSix(void);
...
void FigureOne(void) {
	...
	glPointSize(6.0f);
	glBegin(GL_POINTS);
	for (GLfloat i = 0.0f; i < 4.0f; i+=1.0f) {
		for (GLfloat j = 0.0f; j < 4.0f; j+=1.0f) {
			glVertex3f(i, j, 0.0f);
		}
	}
	glEnd();
...
void FigureTwo(void) {
	...
	for (GLfloat x = 0.0f; x < 3.0f; x += 1.0f) {

		for (GLfloat y = 0.0f; y < 3.0f; y += 1.0f) {
			...
			glVertex3f(0.0f, x+1, 0.0f);
			glVertex3f(3.0f, x+1, 0.0f);
		}
	}
