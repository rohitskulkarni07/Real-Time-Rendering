...
void Tree(void) {
	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			glPointSize(50.0f);
			for (GLfloat i = 0.0f; i < 2.0f; i += 0.1f) {
				glPushMatrix();
...
void Star(void) {
	...
	glRotatef(gfTimTim++, 0.0f, 0.0f, 1.0f);
	glColor4f(1, 1, 0,1.0f);
		glPushMatrix();
			
			quadric = gluNewQuadric();
			gluCylinder(quadric, 0.1, 0.0f, 0.3f, 60, 60);
		glPopMatrix();

		glPushMatrix();

			glRotatef(71.0f, 0.0f, 1.0f, 0.0f);
			quadric = gluNewQuadric();
			gluCylinder(quadric, 0.1, 0.0f, 0.3f, 60, 60);
		...
		glPushMatrix();
			...
			glRotatef(289.0f, 0.0f, 1.0f, 0.0f);
			quadric = gluNewQuadric();
			gluCylinder(quadric, 0.1, 0.0f, 0.3f, 60, 60);
