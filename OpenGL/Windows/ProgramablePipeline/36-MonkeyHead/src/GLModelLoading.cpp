...
void loadOBJModel(const char* filename, Model_Data* model_data, int vertexIndex, int textureIndex, int normalIndex) {
	...
	//code
	if (fopen_s(&gpFile, "logForModelLoading.txt", "w") != 0) {
		MessageBox(NULL, TEXT("FILE CREATION FAILED PROGRAM WILL EXIT..."), TEXT("ERROR"), MB_ICONEXCLAMATION | MB_OK);
		exit(0);
	}
	else {
		fprintf(gpFile, "File \"logForModelLoading.txt\" Created... Program Started Succesfully!\n");
	}

	//open obj file
	fp = fopen(filename, "r");
	if (fp == NULL) {
		fprintf(gpFile, "Failed To Open .Obj File");
		return;
	}
	...
	fprintf(gpFile, "Vertex Array Size:        %d", vert_size);
	fprintf(gpFile, "Number Of Vertex:         %d", vert_size/3);

	fprintf(gpFile, "Texture Array Size:       %d", text_size);
	fprintf(gpFile, "Number Of Texture:        %d", text_size/2);

	fprintf(gpFile, "Normal Array Size:        %d", norm_size);
	fprintf(gpFile, "Number Of Normal:         %d", norm_size/3);

	fprintf(gpFile, "Indices:        %d", index_size);
	fprintf(gpFile, "Faces:          %d", index_size / 3);
