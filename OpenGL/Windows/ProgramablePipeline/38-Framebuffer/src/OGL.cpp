...
GLuint vertexShaderObject_rtt;
GLuint fragmentShaderObject_rtt;
GLuint shaderProgramObject_rtt;
...
GLuint mvpMatrixUniform_rtt;	//for model-view-projection
GLuint textureSamplerUniform_rtt;	//for sampler
...
void initialize(void) {
	...
	//___	glGetShaderiv(vertexShaderObject, GL_COMPILE_STATUS, &shaderCompileStatus);
	vertexShaderObject_rtt = glCreateShader(GL_VERTEX_SHADER);

	const GLchar* vertexShaderSourceCode_rtt =
		"#version 450 core"
		"\n" \
		"in vec4 vPosition;" \
		"in vec2 vTexCoord;" \
	...
	glShaderSource(vertexShaderObject_rtt, 1, (const GLchar**)&vertexShaderSourceCode_rtt, NULL);
	glCompileShader(vertexShaderObject_rtt);
	...
	glGetShaderiv(vertexShaderObject_rtt, GL_COMPILE_STATUS, &shaderCompileStatus);

	if (shaderCompileStatus == GL_FALSE) {

		glGetShaderiv(vertexShaderObject_rtt, GL_INFO_LOG_LENGTH, &infoLogLength);
		...
		if (infoLogLength > 0) {
			...
			if (szInfoLog != NULL) {
				GLsizei written;
				glGetShaderInfoLog(vertexShaderObject_rtt, infoLogLength, &written, szInfoLog);
				fprintf(gpFile, "*** Vertext_rtr Shader Compilation Status***\n%s\n", szInfoLog);
				free(szInfoLog);
				DestroyWindow(ghwnd);
			}
		}
	...
	fragmentShaderObject_rtt = glCreateShader(GL_FRAGMENT_SHADER);

	const GLchar* fragmentShaderSourceCode_rtt =
		"#version 450 core" \
		"\n" \
		"in vec2 out_texcoord;" \
		"uniform sampler2D u_texture_sampler;" \
	...
	glShaderSource(fragmentShaderObject_rtt, 1, (const GLchar**)&fragmentShaderSourceCode_rtt, NULL);

	infoLogLength = 0;
	shaderCompileStatus = 0;
	szInfoLog = NULL;

	glGetShaderiv(fragmentShaderObject_rtt, GL_COMPILE_STATUS, &shaderCompileStatus);

	if (shaderCompileStatus == GL_FALSE) {

		glGetShaderiv(fragmentShaderObject_rtt, GL_INFO_LOG_LENGTH, &infoLogLength);
		...
		if (infoLogLength > 0) {
			...
			if (szInfoLog != NULL) {
				GLsizei written;
				glGetShaderInfoLog(fragmentShaderObject_rtt, infoLogLength, &written, szInfoLog);
				fprintf(gpFile, "*** Fragment Shader Compilation Status***\n%s\n", szInfoLog);
				free(szInfoLog);
				DestroyWindow(ghwnd);
			}
	...
	glCompileShader(fragmentShaderObject_rtt);
	...
	shaderProgramObject_rtt = glCreateProgram();
	glAttachShader(shaderProgramObject_rtt, vertexShaderObject_rtt);
	glAttachShader(shaderProgramObject_rtt, fragmentShaderObject_rtt);
	//pre-link binding of shader program object with vertex shader position attribute
	glBindAttribLocation(shaderProgramObject_rtt, RSK_ATTRIBUTE_POSITION, "vPosition");
	glBindAttribLocation(shaderProgramObject_rtt, RSK_ATTRIBUTE_TEXTURE0, "vTexCoord");
	
	glLinkProgram(shaderProgramObject_rtt);
	...
	//Error checking
	glGetProgramiv(shaderProgramObject_rtt, GL_LINK_STATUS, &shaderProgramLinkStatus);
	if (shaderProgramLinkStatus == GL_FALSE) {

		glGetProgramiv(shaderProgramObject_rtt, GL_INFO_LOG_LENGTH, &infoLogLength);
		...
		if (infoLogLength > 0) {
			...
			if (szInfoLog != NULL) {
				GLsizei written;
				glGetProgramInfoLog(shaderProgramObject_rtt, infoLogLength, &written, szInfoLog);
				fprintf(gpFile, "Shader Program Link Log :\n%s\n", szInfoLog);
	...
	mvpMatrixUniform_rtt = glGetUniformLocation(shaderProgramObject_rtt, "u_mvp_matrix");
	textureSamplerUniform_rtt = glGetUniformLocation(shaderProgramObject_rtt, "u_texture_sampler");
...
void display(void) {
	...
	// code
		// Clear My Window With Color Set by glClearColor() and Set The Buffer Bit With Given Value In Initialize Funtion
	...
	glBindFramebuffer(GL_FRAMEBUFFER, gFrameBuffer);
		...
		// glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
			...
			// glBindTexture(GL_TEXTURE_2D, gColorTexture);
			// glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB,gWidth, gHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
			// glBindTexture(GL_TEXTURE_2D, 0);
			// glBindTexture(GL_TEXTURE_2D, gDepthTexture);
			// glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT32F, gWidth, gHeight);
			// glBindTexture(GL_TEXTURE_2D, 0);
			// glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, gColorTexture, 0);
			// glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, gDepthTexture, 0);
	...
	glUseProgram(shaderProgramObject_rtt);
	...
	translateMatrix = vmath::translate(0.0f, 0.0f, -4.6f);
	//scaleMatrix = vmath::scale(0.75f, 0.75f, 0.75f);
	rotationMatrixX = vmath::rotate(cubeAngle, 0.0f, 0.0f, 1.0f);
	rotationMatrixY = vmath::rotate(cubeAngle, 0.0f, 1.0f, 0.0f);
	rotationMatrixZ = vmath::rotate(cubeAngle, 1.0f, 0.0f, 0.0f);

	modelViewMatrix = translateMatrix * rotationMatrixX * rotationMatrixY * rotationMatrixZ;
	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;

	glUniformMatrix4fv(mvpMatrixUniform_rtt, 1, GL_FALSE, modelViewProjectionMatrix);

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gColorTexture);	//change state to texture for next upcoming geometry
	glUniform1i(textureSamplerUniform_rtt, 0);
...
void update(void) {
	...
	pyramidAngle = pyramidAngle + 0.01f;
	if (pyramidAngle >= 360) {
		pyramidAngle = 0;
	}
	cubeAngle = cubeAngle + 0.01f;
	if (cubeAngle >= 360) {
		cubeAngle = 0;
	}
}
