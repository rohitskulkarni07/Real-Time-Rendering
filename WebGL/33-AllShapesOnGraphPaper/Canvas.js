...
var vao_outer_circle;
var vbo_position_outer_circle;
...
function init() {
    ...
    var outerCircleVertices = new Float32Array(1080);
    angleOfCircle = 0.0;
    circleRadius = gfRadOuter;

    for(let i = 0; i < 360; i++) {
        outerCircleVertices[i*3]     = circleRadius * Math.cos(angleOfCircle);
        outerCircleVertices[i*3 + 1] = circleRadius * Math.sin(angleOfCircle);
        outerCircleVertices[i*3 + 2] = 0.0;
        angleOfCircle += 2 * Math.PI/360;
    }
    ...
    vao_outer_circle = gl.createVertexArray();
    gl.bindVertexArray(vao_outer_circle);

    vbo_position_outer_circle = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vbo_position_outer_circle);
    gl.bufferData(gl.ARRAY_BUFFER,outerCircleVertices, gl.STATIC_DRAW);
    gl.vertexAttribPointer(WebGLMacros.RSK_ATTRIBUTE_VERTEX, 3, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(WebGLMacros.RSK_ATTRIBUTE_VERTEX);
    gl.bindBuffer(gl.ARRAY_BUFFER, null);
...
function draw() {
    ...
    mat4.scale(modelViewMatrix, modelViewMatrix, [1.0 ,0.5, 0.0]);
...
function drawCircle() {
    ...
    mat4.translate(modelViewMatrix, modelViewMatrix, [ix, iy, -2.5]);
    mat4.multiply(modelViewProjectionMatrix, perspectiveProjectMatrix, modelViewMatrix);
